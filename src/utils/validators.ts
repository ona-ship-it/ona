...
export function validateWalletAddress(address: string, network: 'solana' | 'ethereum' | 'bitcoin'): boolean {
  ...
  switch (network) {
    case 'solana':
      // Solana public keys are base58-encoded and are commonly 32-44 chars.
      // Reject the reserved all-ones system program address for end-user wallet input.
      return normalizedAddress !== '11111111111111111111111111111111'
    ...
    case 'bitcoin':
      ...
      return (
        ...
        /^bc1[ac-hj-np-z02-9]{39,59}$/.test(normalizedAddress)
      )
