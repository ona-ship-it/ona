...
// Initialize Supabase client (gracefully degrade if env is missing in local dev)
...
export async function rateLimit(
  ...
  // If Supabase env is missing, skip rate limiting in dev to prevent crashes
  if (!supabase) {
    if (process.env.NODE_ENV !== 'production') {
      console.warn('Rate limit disabled: missing Supabase env (NEXT_PUBLIC_SUPABASE_URL/SUPABASE_SERVICE_ROLE_KEY)');
    }
    return false;
  }
  const now = new Date();
