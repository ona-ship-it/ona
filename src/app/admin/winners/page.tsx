...
export default function AdminWinnersPage() {
  ...
  const fetchTickets = async (giveawayId: string) => {
    try {
      const { data, error } = await supabase
        .from('tickets')
  ...
  const handleGiveawaySelect = (giveawayId: string) => {
  ...
  const drawWinner = async () => {
    ...
    // Animate through random tickets for suspense
    let count = 0
