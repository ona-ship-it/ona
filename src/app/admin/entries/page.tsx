  ...
  async function fetchEntries() {
    ...
    try {
      const { data, error } = await supabase
        .from('tickets')
        .select('id, created_at, purchase_price, payment_currency, is_winner, user_id, giveaway_id')
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        ...
        {loading ? (
          ...
          <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-3xl p-12 text-center">
            ...
            <p className="text-slate-400">No entries have been made to any giveaways</p>
          </div>
        ) : (
                ...
                {entries.map((entry, index) => (
                  <tr key={entry.id} className={index % 2 === 0 ? 'bg-slate-800/20' : ''}>
                    <td className="px-6 py-4">
                      <div className="text-white font-semibold">{entry.profile_name}</div>
                      <div className="text-slate-400 text-sm">{entry.profile_email}</div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <span className="text-2xl">{entry.giveaway_emoji}</span>
                        <span className="text-white">{entry.giveaway_title}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
