...
export default function GiveawaysPage() { 
  ...
  async function joinFree(giveawayId: string) { 
    ...
    try { 
      ...
      toast.success("Free ticket claimed — good luck!"); 
      fetchGiveaways(); 
    } catch (e: any) { 
      console.error(e); 
      toast.error(e.message || "Failed to claim free ticket"); 
    } 
  } 
  ...
  async function buyTicket(giveawayId: string, qty = 1) { 
    ...
    try { 
      ...
      toast.success(`Bought ${qty} ticket(s). Good luck!`); 
  ...
  async function donate(giveawayId: string) { 
    try { 
      ...
      if (!amount || amount <= 0) { 
        toast.error("Enter a valid donation amount (USDT)."); 
        return; 
      } 
      ...
      toast.success("Thanks for donating — your donation was added to the prize pool."); 
