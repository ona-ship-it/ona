...
export default function NewGiveawayPage() { 
  ...
  async function handleCreate(e?: React.FormEvent) { 
    ...
    try { 
      ...
      if (userIsAdmin && activateImmediately) {
        toast.success("Giveaway created and activated immediately (admin privilege — no escrow required).");
      } else {
        toast.success("Giveaway created as draft. Activate it to lock escrow.");
      }
  ...
  async function activateGiveaway(giveawayId: string) { 
    ...
    try { 
      ...
      if (adminStatus) {
        toast.success("Giveaway activated with admin privileges — no escrow required.");
      } else {
        toast.success("Giveaway activated — prize escrowed if funds available.");
      }
      
      router.push("/giveaways"); 
    } catch (err: any) { 
      console.error(err); 
      toast.error(err.message || "Failed to activate giveaway"); 
    } finally { 
      setLoading(false); 
    } 
  } 
